...
export async function sendProjectSubmissionEmail(supervisorEmail, studentName, projectTitle, projectId, isResubmission = false) {
  ...
  let subject, html;
  if (isResubmission) {
    subject = `Resubmission: Project Draft for Review: ${projectTitle}`;
    html = `<p>Hello,</p><p>Student <strong>${studentName}</strong> has <strong>resubmitted</strong> a project draft titled "<strong>${projectTitle}</strong>" for your review.</p><p>Please review it here: <a href="${reviewLink}">${reviewLink}</a></p>`;
  } else {
...
export async function sendProjectStatusUpdateEmail(studentEmail, studentName, projectTitle, status, comments = '') {
  const currentTransporter = await getTransporter();
  const projectLink = `${process.env.NEXTAUTH_URL}/`;
  const senderEmail = process.env.EMAIL_FROM || `"Project Repository" <noreply@yourdomain.com>`;
...
export async function sendOtpEmail(to, otp) {
  ...
  const subject = 'Your OTP Code for Student Registration';
  const html = `<p>Hello,</p><p>Your One-Time Password (OTP) for registration is:</p><h2>${otp}</h2><p>This code is valid for a limited time. If you did not request this, please ignore this email.</p>`;
